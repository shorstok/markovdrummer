...
using markov_drummer.Markov.Chiscore.Models;

namespace markov_drummer.Markov.Chiscore
{
    /// <summary>
    ...
    public abstract class GenericMarkov<TPhrase, TUnigram> : IMarkovStrategy<TPhrase, TUnigram>
    {
        ...
        {
            if (phrase == null || phrase.Equals(default(TPhrase))) return;

            // Ignore particularly short phrases
            if (SplitTokens(phrase).Count() < Level) return;

            // Add it to the source lines so we can ignore it 
            // when learning in future
            if (!SourcePhrases.Contains(phrase)) SourcePhrases.Add(phrase);
            ...
            {
                var index = tokens.Length - j;

                var previous = index >= 0 && index < tokens.Length ? tokens[tokens.Length - j] : GetPrepadUnigram();
        ...
        {
            if (seed == null) seed = RebuildPhrase(new List<TUnigram> {GetPrepadUnigram()});
            ...
            while (created < lines)
            {
                if (genCount == lines * 10) break;
                var result = WalkLine(seed);
                if ((!EnsureUniqueWalk || !SourcePhrases.Contains(result)) &&
        ...
        public List<TUnigram> GetMatches(TPhrase input)
        {
            ...
            else if (inputArray.Length < Level) inputArray = PadArrayLow(inputArray);


            return Chain.GetValuesForKey(new NgramContainer<TUnigram>(inputArray));
        }
        ...
        public virtual ChainPhraseProbability<TPhrase> GetFit(TPhrase testData)
        {
            ...
            for (var i = 0; i < testUnigrams.Length; i++)
            {
                ...
                {
                    var index = i + j;
                    if (index >= testUnigrams.Length) break;
                ...
                if (testCase.Count == Level)
                {
                    ...
                    try
                    {
                        ...
                        results.Add(testResults.Any()
                            ? new NgramChainMatch<TPhrase>(testPhrase, true)
                            : new NgramChainMatch<TPhrase>(testPhrase, false));
                    }
        ...
        public double GetTransitionProbabilityPhrase(TPhrase currentState, TPhrase nextStates)
        {
            ...
            foreach (var followingState in SplitTokens(nextStates))
            {
                ...
                if (Math.Abs(pTotal) < float.Epsilon)
                    break;
        ...
        /// <summary>
        ///     Iterate over a list of TGrams and store each of them in the model at a composite key genreated from its prior
        ///     [Level] number of TGrams
        /// </summary>
        /// <param name="tokens"></param>
        ...
        private TPhrase WalkLine(TPhrase seed)
        {
            ...
            while (built.Count < 1500)
            {
                ...
                {
                    var chosen = built.Count == 0
                        ? new UnweightedRandomUnigramSelector<TUnigram>().SelectUnigram(Chain.GetValuesForKey(key))
                        : UnigramSelector.SelectUnigram(Chain.GetValuesForKey(key));
        ...
        {
            if (input == null) input = new List<TUnigram>().ToArray();

            var splitCount = input.Length;
            if (splitCount > Level) input = input.Skip(splitCount - Level).Take(Level).ToArray();
            ...
            for (var i = Level - input.Length; i > 0; i--) p[i - 1] = GetPrepadUnigram();
